...
def bbox2mask(bboxes, height, width, max_delta_h, max_delta_w):
    ...
    for i in range(batch_size):
        ...
        mask_location.append((bbox[0] + delta_h, bbox[0] + bbox[2] - delta_h, bbox[1] + delta_w, bbox[1] + bbox[3] - delta_w))  # (x1,x2,y1,y2)
    return mask, torch.tensor(mask_location, dtype=torch.int64)
...
def generate_stroke_mask(im_size, parts=40, maxVertex=int(20/2), maxLength=int(100/2), maxBrushWidth=int(24/2), maxAngle=360):
...
def mask_image(x, bboxes, config, is_valing=False):
    batch_size = bboxes.size(0)
    height, width, _ = config['image_shape']
    mask = torch.zeros((batch_size, 1, height, width), dtype=torch.float32)
    spatial_discounting_mask_tensor = torch.zeros((batch_size, 1, height, width), dtype=torch.float32)
    for i in range(batch_size):
        bbox = bboxes[i]  # the location of per 128*128 region, (x1, y1, h=128, w=128)

        # create 128*128 irregular mask
        irr_mask = generate_stroke_mask((128, 128))
        irr_mask128 = np.squeeze(irr_mask, 2)

        # create 256*256 whole mask (put 128*128 irregular mask into the 256*256 whole mask)
        mask[i][0, bbox[0]:bbox[0]+bbox[2], bbox[1]:bbox[1]+bbox[3]] = torch.tensor(irr_mask128, dtype=torch.float32)

        mask_values128 = np.zeros((128, 128))  # mask_values128 is the 128*128 weigt mask
        _mask1 = irr_mask128.copy()
        _mask2 = irr_mask128.copy()

        mask_values128 = np.pad(mask_values128, pad_width=((1, 1), (1, 1)), mode='constant', constant_values=(0, 0))
        _mask1 = np.pad(_mask1, pad_width=((1, 1), (1, 1)), mode='constant', constant_values=(0, 0))
        _mask2 = np.pad(_mask2, pad_width=((1, 1), (1, 1)), mode='constant', constant_values=(0, 0))

        k = 0
        while (np.sum(_mask1) != 0.):
            k += 1
            kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
            _mask1 = cv2.erode(_mask1, kernel)
            mask_values128 = mask_values128 + ((_mask2 - _mask1) * np.power(0.99, k - 1))
            _mask2 = _mask1

        mask_values128 = mask_values128[1:1 + 128, 1:1 + 128]

        # put 128 * 128  mask_values128 into the 256 * 256 whole weigt mask
        spatial_discounting_mask_tensor[i][0, bbox[0]:bbox[0]+bbox[2], bbox[1]:bbox[1]+bbox[3]] = torch.tensor(mask_values128, dtype=torch.float32)
    ...
    return result, mask, spatial_discounting_mask_tensor



def my_discounting_mask(config, mask_locations):
    batch_size = mask_locations.size(0)
    gamma = config['spatial_discounting_gamma']
    height, width, _ = config['image_shape']  #  256, 256

    shape = [1, 1, height, width]
    if config['discounted_mask']:
        for i in range(batch_size):
            mask_location = mask_locations[i]  # x1,x2,y1,y2
            x1 = mask_location[0].cpu().numpy()
            x2 = mask_location[1].cpu().numpy()
            y1 = mask_location[2].cpu().numpy()
            y2 = mask_location[3].cpu().numpy()

            mask_values = np.zeros((height, width))
            irr_random_ratio = random.choice([256, 512])
            irr_mask = generate_stroke_mask((irr_random_ratio, irr_random_ratio))
            irr_mask = np.squeeze(irr_mask, 2)
            irr_mask128 = cv2.resize(irr_mask, (128, 128), interpolation=cv2.INTER_NEAREST)
