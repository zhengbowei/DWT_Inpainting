...
from utils.tools import get_config, random_bbox, mask_image, local_patch
...
def main():
    ...
    try:  # for unexpected error logging
        ...
        for iteration in range(start_iteration, config['niter'] + 1):
            ...
            losses, inpainted_result = trainer(x, bboxes, mask, ground_truth, spatial_discounting_mask_tensor, compute_g_loss)
            ...
            if compute_g_loss:
                ...
                losses['g'] = losses['Context'] * 10.0 \
                              + losses['DWT'] * 10.0 \
                              + losses['wgan_g'] * 0.001 \
                              + losses['Perceptual'] * 1.0 \
                              + losses['Style'] * 1000
                losses['g'].backward()
                trainer_module.optimizer_g.step()

            # Log and visualization
            log_losses = ['Context', 'DWT', 'Perceptual', 'Style', 'wgan_g', 'wgan_d', 'wgan_gp', 'g', 'd']
            ...
            if iteration % (config['viz_iter']) == 0:
                ...
                viz_images = viz_images.cpu().data
